  ...
  async CreateRecords(req, res, next) {
    try {
      ...
      if (!participant) {
        return res
          .status(403)
          .json({ message: '존재하지 않는 그룹원이거나 닉네임이 잘못되었습니다.' });
      }
      ...
      const webhookUrl = participant.group?.discordwebhookurl;
      if (webhookUrl) {
        const recordData = {
          nickname: authorNickname,
          exerciseType,
        ...
        // 위에서 만든 함수 호출
        await sendDiscordNotification(webhookUrl, recordData, participant.group.groupName);
      }
  ...
  async GetRecords(req, res, next) {
    try {
      ...
      const where = {
        groupId: id,
        //닉네임으로 검색
        authorNickname: search ? { contains: search } : undefined,
      };
